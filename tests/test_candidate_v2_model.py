...
from coal_forecasting.candidate import fit_predict_v2_ridge


class CandidateV2ModelTests(unittest.TestCase):
    def test_v2_is_deterministic_nonnegative_and_has_zero_intercept(self) -> None:
        ...
        first = fit_predict_v2_ridge(training, prediction, alpha=100.0)
        second = fit_predict_v2_ridge(training, prediction, alpha=100.0)
    ...
    def test_v2_large_alpha_shrinks_toward_persistence(self) -> None:
        training = pd.DataFrame(
            {
        ...
        small = fit_predict_v2_ridge(training, prediction, alpha=1.0)
        large = fit_predict_v2_ridge(training, prediction, alpha=1_000_000.0)

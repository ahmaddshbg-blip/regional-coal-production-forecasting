# Persistence-Anchored Raw-Delta Ridge Candidate Procedure
...
The raw-delta candidate passed all implementation-validity and coverage checks
but did not pass the frozen promotion criteria. The selected alpha was `10000`. Equal-weight mean
...
The raw-delta specification removed the log-change candidate's severe
underforecasting and produced
...
The log-change candidate failed before confirmation because an equally
weighted log-change loss produced severe raw-ton underforecasting in
high-volume states and left strong long-horizon residual dependence. The
raw-delta candidate tests one bounded response to that diagnosis: learn a
direct raw-ton correction to persistence while retaining a strongly
regularized global linear model.
...
Keep the log-change procedure's prequential horizon-specific conformal
intervals: eight-origin
...
closed. If the raw-delta candidate does not materially improve on the frozen
comparator or exposes a material model-working failure, retain the baseline
and stop; no third candidate is authorized.
